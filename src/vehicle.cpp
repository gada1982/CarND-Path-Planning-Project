// Solution from https://github.com/subhash/CarND-Behaviour-Planning/blob/master/src/vehicle.cpp
// Mentioned here https://discussions.udacity.com/t/path-planning-lecture-4-16/306201/43
...
float cost_for_action(map<int,vector < vector<int> > > predictions, string action, int current_lane, int target_lane, int available_lanes, int target_speed){
  ...
  for (auto const& entry: predictions) {
    ...
    int vl = entry.second[0][0];
    int vs = entry.second[0][1];
    int gap = vs - s;
...
void Vehicle::update_state(map<int,vector < vector<int> > > predictions) {
  ...
  int current_lane = predictions[-1][0][0];
  int target_lane = goal_lane;
  
  // Define the possible actions depanding on the actual state
