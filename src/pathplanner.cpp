                         ...
                         vector<double> previous_path_x, vector<double> previous_path_y, double end_path_s, double end_path_d)
{
  ...
  bool simpleLine1 = true;
  if(simpleLine1 == true)
  {
    double dist_inc = 0.5;
    for(int i = 0; i < 50; i++)
    {
      new_path.push_back(car_x+(dist_inc*i)*cos(deg2rad(car_yaw)));
      new_path.push_back(car_y+(dist_inc*i)*sin(deg2rad(car_yaw)));
    }
  }
  
  bool followSpline1 = false;
  if(simpleLine1 == false && followSpline1 == true)
  {
